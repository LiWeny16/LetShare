...
async function main() {
  ...
  try {
    ...
    await a.screenshot({ path: ".e2e-pro-A.png" });
    ...
    await b.screenshot({ path: ".e2e-pro-B.png" });
    ...
    await b.screenshot({ path: ".e2e-pro-B-chat.png" });
    ...
    await a.waitForTimeout(600);
    await a.screenshot({ path: ".e2e-pro-A-wb.png" });
    ...
    ok("B-wb-recv", bInk);
    await b.screenshot({ path: ".e2e-pro-B-wb.png" });
    ...
    await a.screenshot({ path: ".e2e-pro-A-share.png" });
    await b.waitForTimeout(2500);
    // B 远端视频轨 = 摄像头 + 屏幕共享（重协商扇出）
    const bVideoTracks = await b.evaluate(() =>
      window.__meeting ? window.__meeting.getState().remoteTracks.filter((t) => t.kind === "video").length : -1
    ).catch(() => -1);
    const bVideosAfterShare = await b.locator("video").count();
    ok("B-share-recv", bVideoTracks === 2 && bVideosAfterShare > bVideoCount, `videoTracks=${bVideoTracks} videos ${bVideoCount}→${bVideosAfterShare}`);
    await b.screenshot({ path: ".e2e-pro-B-share.png" });
    ...
    await c.screenshot({ path: ".e2e-pro-C-kicked.png" });
    await ctxC.close();
    ...
    await b.screenshot({ path: ".e2e-pro-B-breakout.png" });
    ...
    await b.screenshot({ path: ".e2e-pro-B-recall.png" });
    ...
    await b.screenshot({ path: ".e2e-pro-B-ended.png" });
    ...
    await d.screenshot({ path: ".e2e-pro-D-404.png" });
    await ctxD.close();
