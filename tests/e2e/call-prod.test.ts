/**
 * 生产环境 E2E：双客户端经 ecs.letshare.fun（WS 信令 + 生产 SFU）语音通话。
 *
 ...
 *   - 断言同本地版：双端 SFU transport、音频字节递增
 *
...
test("生产环境：双客户端经 ecs.letshare.fun SFU 完成语音通话", async (t) => {
  // ── 0. 前置：Pages 已部署本次构建 ────────────────────────────────
  ...
  // ── 1. 双客户端（生产站，custom 服务器模式，启用 SFU 调试 stats）────
  ...
  // ── 3. 断言：SFU 媒体链路 + 双向音频字节 + 语音电平 + 丢包 ────────
  // 单通（"听得到对方、对方听不到我"）的三层定位：
  //   层1 字节/包：bytesSent/bytesReceived、packetsReceived 递增 —— 包确实在双向流动（网络/中继 OK）
  //   层2 电平：audioLevel 作为诊断信息保留；Chromium fake microphone 可能恒为 0，不能作为链路硬断言
  ...
  for (const [i, page] of pages.entries()) {
    try {
      ...
      throw new Error(`${error instanceof Error ? error.message : String(error)} stats=${JSON.stringify(statsDiagnostic)}`);
    }
  }
  ...
  for (const [i, page] of pages.entries()) {
    ...
    // 电平只打印诊断：fake microphone 的合成采样可能是静音，但不影响 RTP 链路验收。
