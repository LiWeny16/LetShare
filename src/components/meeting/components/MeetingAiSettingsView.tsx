...
import XiaomiIcon from "@thesvg/react/xiaomi";
      ...
      <Stack spacing={1.25} sx={{ maxWidth: 680, mx: "auto" }}>
        {isHost ? <>
          <Paper elevation={0} sx={{ p: { xs: 1.35, sm: 1.6 }, border: "1px solid #e4e7ec", borderRadius: 2, bgcolor: "#fff" }}>
            <Stack spacing={1.15}>
              ...
              <FormControl fullWidth size="small" sx={fieldSx}><InputLabel>服务商</InputLabel><Select value={config.summaryProvider} label="服务商" onChange={(event) => onChooseProvider(event.target.value as SummaryProvider)} renderValue={(value) => { const current = SUMMARY_PROVIDERS.find((item) => item.id === value) ?? SUMMARY_PROVIDERS[0]; return <Stack direction="row" spacing={1} alignItems="center"><ProviderMark provider={current.id} /><Box component="span">{current.label}</Box></Stack>; }}>{SUMMARY_PROVIDERS.map((provider) => <MenuItem key={provider.id} value={provider.id}><ProviderMark provider={provider.id} />{provider.label}</MenuItem>)}</Select></FormControl>
              ...
              {config.summaryProvider === "mimo" && <FormControl fullWidth size="small" sx={fieldSx}><InputLabel>服务区域</InputLabel><Select value={mimoEndpoint} label="服务区域" onChange={(event) => onUpdateConfig({ summaryBaseUrl: event.target.value === "api" ? MIMO_API_BASE_URL : MIMO_TOKEN_PLAN_BASE_URL })}><MenuItem value="token-plan"><Stack direction="row" spacing={1} alignItems="center"><PublicIcon sx={{ fontSize: 18, color: "#ff6900" }} /><Box component="span">Token Plan · 中国</Box></Stack></MenuItem><MenuItem value="api"><Stack direction="row" spacing={1} alignItems="center"><ApiIcon sx={{ fontSize: 18, color: "#475467" }} /><Box component="span">标准 API</Box></Stack></MenuItem></Select></FormControl>}
        ...
        <Paper elevation={0} sx={{ p: { xs: 1.35, sm: 1.6 }, border: "1px solid #e4e7ec", borderRadius: 2, bgcolor: "#fff" }}>
          <Stack spacing={1.1}>
            ...
            {(isHost ? config.asrSource : memberAsrSource) === "mimo-asr" && <TextField size="small" fullWidth sx={fieldSx} label="本机 MiMo API Key" type="password" value={memberApiKey} onChange={(event) => onMemberApiKeyChange(event.target.value)} autoComplete="off" helperText={memberApiKey.trim() ? "只保存在当前标签页" : "填写后才能使用本机 MiMo ASR"} />}
            {isHost && config.asrSource === "mimo-asr" && <Alert severity={sourceReady ? "success" : "warning"} icon={<GraphicEqIcon fontSize="small" />} sx={{ borderRadius: 1.5, fontSize: "0.7rem" }}>{sourceReady ? "本机 MiMo ASR 已准备好。" : "请先填写本机 MiMo API Key。"}</Alert>}
