...
interface ReceiveSession {
 ...
 totalChunks: number;
 chunkSize: number;
 /** 预分配缓冲区（小文件兼容） — 接受前为 null */
 buffer: Uint8Array | null;
 /** 分块收集 — 避免大文件 OOM，最后用 Blob 组装 */
...
export class ServerFileTransfer {
 ...
 public handleBinaryData(data: ArrayBuffer) {
  ...
  try {
   ...
   if (!session || session.status !== "receiving") {
    ...
    console.warn(`[ServerFileTransfer] ${reason} transfer=${meta.transfer_id}`);
 ...
 private writeChunkToSession(
  ...
  if (session.receivedChunkIndexes.has(chunkIndex)) {
   console.warn(`[ServerFileTransfer] duplicate chunk ignored: ${chunkIndex} transfer=${session.transferId}`);
   return;
  }
 ...
 /**
  * 处理传输块元数据 — Bug1 修复: 将真实 chunk_index 存入 session, 供下一个二进制帧使用
  */
 private handleTransferChunk(chunkMeta: FileTransferChunk) {
  const session = this.receivingSessions.get(chunkMeta.transfer_id);
  if (!session) {
   console.warn(`[ServerFileTransfer] handleTransferChunk: 未找到会话 ${chunkMeta.transfer_id}`);
   return;
  }
